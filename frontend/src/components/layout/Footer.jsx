      ...
      <VintageContainer className="mx-auto max-w-[1400px]" variant="flat" padding="lg">
        <div className="grid gap-8 md:grid-cols-3">
          ...
          <div className="flex flex-col gap-3 text-sm text-slate-700">
            {navLinks.map((link) => (
              <a key={link.to} href={link.to} className="rounded-full px-4 py-2 hover:text-slate-900 hover:bg-[#FFE7C2] retro-shadow">
                {link.label}
              </a>
            ))}
          </div>
          ...
          <div className="flex flex-col gap-3 text-sm text-slate-700">
            <a href="https://github.com/Arbab-ofc/AI-VIRTUAL-MOUSE" target="_blank" rel="noreferrer" className="flex items-center gap-2 rounded-full px-4 py-2 hover:text-slate-900 hover:bg-[#FFE7C2] retro-shadow">
