      ...
      <VintageContainer className="mx-auto max-w-[1400px]" variant="flat" padding="lg">
        <div className="grid gap-8 md:grid-cols-3">
          ...
          <div className="flex flex-col gap-3 text-sm text-slate-600">
            {navLinks.map((link) => (
              <a key={link.to} href={link.to} className="hover:text-blue-600">
                {link.label}
              </a>
            ))}
          </div>
          ...
          <div className="flex flex-col gap-3 text-sm text-slate-600">
            <a href="https://github.com" target="_blank" rel="noreferrer" className="flex items-center gap-2 hover:text-blue-600">
